...
//////////////////////////////////
// ฟังก์ชันดึงข้อมูลที่อยู่จาก localStorage
function getSavedAddresses() {
  return JSON.parse(localStorage.getItem('addresses')) || [];
}
 
// ฟังก์ชันสร้าง HTML ของ address card
function createAddressCard(address, index) {
  ...
  radio.type = 'radio';
  radio.name = 'address';
  if(index === 0) radio.checked = true; // เลือกตัวแรกเป็น default
  ...
  title.textContent = address.type || 'ที่อยู่';
 
  const name = document.createElement('div');
  name.className = 'address-name';
  name.textContent = `${address.fname} ${address.lname}`;
 
  const details = document.createElement('div');
  details.className = 'address-details';
  details.innerHTML = `
    ${address.houseNo}, หมู่ ${address.village}, ${address.subdistrict}<br>
    ${address.district}, ${address.province}, ${address.zipcode}<br>
    โทร: ${address.phone}
  `;
...
function renderAddresses() {
  ...
  addressList.innerHTML = ''; // ล้างข้อมูลเก่า
  const savedAddresses = getSavedAddresses();
 
  if(savedAddresses.length === 0){
    addressList.innerHTML = '<p>ยังไม่มีที่อยู่บันทึกไว้ <a href="../addaddress/address.html">+ เพิ่มที่อยู่ใหม่</a></p>';
    return;
  }
