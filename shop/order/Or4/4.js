// ========================
// EASYPRINT — Orders หน้า Delivered / Preparing
// ========================
...
// --- คำนวณจำนวนแต่ละ status ---
function EP_computeCounts(orders) {
  const counts = { all:0, waiting:0, confirmed:0, printing:0, preparing:0, delivered:0, sent:0 };
...
// --- Update numbers shown on filter buttons ---
...
// ========================
// Render Orders Delivered / Preparing
// ========================
function renderDeliveredOrders() {
  const tbody = document.querySelector(".order-table tbody");
  tbody.innerHTML = "";

  const orders = EP_getOrders(); // โหลดสดทุกครั้ง
  Object.keys(orders).forEach(orderId => {
    const data = orders[orderId];
    if (!["delivered","preparing"].includes((data.status||"").toLowerCase())) return;
...
// ========================
// Popup Payment (Event Delegation)
// ========================
